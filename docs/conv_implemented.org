...
#+BEGIN_CENTER
#+NAME: 图 4
#+CAPTION: 均值初始化,学习率设置为 0.1,100 轮训练结果.模型收敛速度较慢.测试准确性只有 70%.
[[file:assets/acc_loss/lenet-uniform-01-loss-acc.png]]
#+END_CENTER
...
#+BEGIN_CENTER
#+NAME: 图 5
#+CAPTION: 均值初始化,学习率设置为 1.0,100 轮训练结果.可看到模型收敛速度加快.测试集上准确性有 90%
[[file:assets/acc_loss/lenet-uniform-10-loss-acc.png]]
#+END_CENTER

可以看到两个模型都还没有出现过拟合现象.分别保持学习率接着训练.
